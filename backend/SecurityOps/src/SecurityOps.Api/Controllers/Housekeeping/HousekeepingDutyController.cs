...
using SecurityOps.Application.Features.Housekeeping.Duty;
...
public sealed class HousekeepingDutyController : ControllerBase
{
    ...
    public HousekeepingDutyController(IMediator mediator) => _mediator = mediator;

    [HttpGet("ping")]
    [AllowAnonymous]
    public ActionResult<ApiResponse<object>> Ping() =>
        Ok(ApiResponse<object>.Ok(new { duty = true, module = "housekeeping" }));
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var created = await _mediator.Send(new CheckInHousekeepingDutyCommand(body), cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var updated = await _mediator.Send(
                new CheckOutHousekeepingDutyCommand(sessionId, body),
                cancellationToken);
    ...
    public async Task<ActionResult<ApiResponse<IReadOnlyList<DutySessionResponse>>>> OnDuty(
        CancellationToken cancellationToken)
    {
        var list = await _mediator.Send(new GetOnDutyHousekeepingSessionsQuery(), cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        var list = await _mediator.Send(
            new GetHousekeepingDutySessionsQuery(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var session = await _mediator.Send(
            new GetOpenHousekeepingDutySessionForStaffQuery(staffName),
            cancellationToken);
