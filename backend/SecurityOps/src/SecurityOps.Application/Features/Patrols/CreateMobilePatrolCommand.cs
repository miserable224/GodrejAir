...
public sealed class CreateMobilePatrolCommandHandler : IRequestHandler<CreateMobilePatrolCommand, PatrolResponse>
{
    ...
    public async Task<PatrolResponse> Handle(CreateMobilePatrolCommand request, CancellationToken cancellationToken)
    {
        ...
        var patrolStaff = await SecurityOpsResolver.ResolveStaffByNameOrBadgeAsync(_db, body.StaffName, cancellationToken);
        if (patrolStaff is null)
            throw new InvalidOperationException($"Staff not found: {body.StaffName}. Add them to the roster or check the name.");
