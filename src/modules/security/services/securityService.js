...
function apiNetworkHint() {
  return `Cannot reach Security API at ${resolveSecurityApiOrigin() ?? 'unknown'}. Set EXPO_PUBLIC_SECURITY_API_URL or EXPO_PUBLIC_API_URL.`;
}
 ...
 */
/**
 * Period dashboard (today / week / month / custom) — deployment totals per role.
 */
export async function fetchSecurityOpsDashboard(token, dateParams, signal) {
  if (!dateParams?.valid || !dateParams.from || !dateParams.to) {
    throw new Error('Select a valid date range.');
  }

  const cacheKey = `sec-ops:${dateParams.cacheKey}`;
  const cached = getCached(cacheKey);
  if (cached) return cached;
  ...
  const body = await apiFetch(`/security/dashboard/period?${qs.toString()}`, token, signal);
  const raw = body?.data ?? body;
  const data = normalizeSecurityOpsDashboard(raw);
  setCache(cacheKey, data);
  return data;
}

function normalizeSecurityOpsDashboard(raw) {
  if (!raw) return null;
...
export async function fetchSecurityDashboard(token, dateParams, signal) {
  ...
  const settled = await Promise.allSettled([
    ...
    fetchSecurityOpsDashboard(token, dateParams, signal),
    dateParams?.mode === 'month' ? fetchMonthlyBilling(token, month, signal) : Promise.resolve(null),
  ]);
  ...
  const payload = {
    ...
    opsDashboard: unwrap(settled[3], 'opsDashboard'),
    billing: unwrap(settled[4], 'billing'),
    errors,
  };
...
/**
 * Save all queued security logs in one multipart POST (photos + metadata together).
 * @param {{ designation: string, staffName: string, location: string, photo: object }[]} items
 */
export async function postBulkSecurityDeployments(token, dateIso, items, signal) {
  const accessToken = token || (await ensureValidAccessToken());
  if (!accessToken) throw new AuthError('Not signed in');
  if (!items?.length) throw new Error('Add at least one security entry before saving.');
  ...
  const res = await fetchWithNetworkHint(
    `${apiBase()}/security/attendance/bulk-deployment-logs`,
    {
      method: 'POST',
      headers: { Authorization: `Bearer ${accessToken}` },
      body: formData,
  ...
  if (!res.ok) {
    throw new Error(`Failed to save security logs (${res.status}): ${await readApiError(res)}`);
  }
...
export async function postMobilePatrol(token, payload, signal) {
  ...
  if (!photo?.uri && !photo?.file) {
    throw new Error('Patrol photo is required.');
  }
